...
def detect_zone_E_and_visualise(session_state,
    ...
    # Upper-storey footprint in plan coordinates using the same axes as the top plane:
    ...
    # B1 for North/South uses EW_dimension minus east+west offsets (i.e. width in x)
    ...
    # North elevation (y = upper_y0) - East edge check (only if north_offset > 0)
    if e1_NS > 0 and east_offset < 0.2 * e1_NS and north_offset > 0:
        rect_w = e1_NS / 5.0   # width along x (west-east)
        rect_h = e1_NS / 3.0   # vertical height (z)
        x1 = upper_x1  # East edge
        x0 = x1 - rect_w
        y0 = upper_y0  # North edge
        y1 = y0 + min(upper_width_y, rect_h)  # extend southwards into the inset by up to rect_h
    ...
    # South elevation (y = upper_y1) - East edge check (only if south_offset > 0)
    if e1_NS > 0 and east_offset < 0.2 * e1_NS and south_offset > 0:
        ...
        x0 = x1 - rect_w
        y1 = upper_y1
        y0 = y1 - min(upper_width_y, rect_h)  # extend northwards into the inset
        clamped = clamp_rect(x0, x1, y0, y1)
        if clamped:
    ...
    # B1 for East/West uses NS_dimension minus north+south offsets (i.e. length in y)
    ...
    # East elevation (x = upper_x1) - North edge check (only if east_offset > 0)
    if e1_EW > 0 and north_offset < 0.2 * e1_EW and east_offset > 0:
        rect_w = e1_EW / 5.0   # width along y (north-south)
        rect_h = e1_EW / 3.0
        y0 = upper_y0  # North edge
        y1 = y0 + rect_w
        x1 = upper_x1  # East edge
        x0 = x1 - min(upper_width_x, rect_h)  # extend westwards into the inset
        clamped = clamp_rect(x0, x1, y0, y1)
        if clamped:
    ...
    if e1_EW > 0 and north_offset < 0.2 * e1_EW and west_offset > 0:
        ...
        y1 = y0 + rect_w
        x0 = upper_x0
        x1 = x0 + min(upper_width_x, rect_h)
        clamped = clamp_rect(x0, x1, y0, y1)
        if clamped:
    ...
    # Draw upper inset box (if positive footprint + H1)
    if upper_width_x > 0 and upper_width_y > 0 and H1 > 0:
        ...
        # bottom, top faces (x=west-east, y=north-south)
        fig.add_trace(go.Mesh3d(
            x=[ux0, ux1, ux1, ux0],
            y=[uy0, uy0, uy1, uy1],
            z=[bz + pad]*4,
        ...
        fig.add_trace(go.Scatter3d(
            x=[ux0, ux1, ux1, ux0, ux0],
            y=[uy0, uy0, uy1, uy1, uy0],
            z=[tz]*5, mode='lines', line=dict(color='black', width=1),
            hoverinfo='none', showlegend=False
        ))

    # Draw each Zone E as a vertical rectangle using the same axis mapping
    for (cx0, cx1, cy0, cy1, rect_h, label) in zoneE_rects:
        bottom_z = top_z
        top_z_rect = top_z + rect_h

        if "North" in label:
            # North elevation - plane at y = cy0 (north edge), x spans cx0..cx1
            fig.add_trace(go.Mesh3d(
                x=[cx0, cx1, cx1, cx0],
                y=[cy0, cy0, cy0, cy0],
            ...
            # South elevation - plane at y = cy1 (south edge)
            fig.add_trace(go.Mesh3d(
                x=[cx0, cx1, cx1, cx0],
                y=[cy1, cy1, cy1, cy1],
                z=[bottom_z, bottom_z, top_z_rect, top_z_rect],
            ...
            # East elevation - plane at x = cx1 (east edge), y spans cy0..cy1
            fig.add_trace(go.Mesh3d(
                x=[cx1, cx1, cx1, cx1],
                y=[cy0, cy1, cy1, cy0],
                z=[bottom_z, bottom_z, top_z_rect, top_z_rect],
            ...
            # West elevation - plane at x = cx0 (west edge)
            fig.add_trace(go.Mesh3d(
                x=[cx0, cx0, cx0, cx0],
                y=[cy0, cy1, cy1, cy0],
                z=[bottom_z, bottom_z, top_z_rect, top_z_rect],
    ...
    # Direction labels (positioned using x=west-east, y=north-south)
    ...
    if upper_width_x > 0 and upper_width_y > 0:
        ...
        label_positions = {
            "North": {"pos": [ux_center, upper_y0 - label_margin, top_z], "text": "N"},
            "South": {"pos": [ux_center, upper_y1 + label_margin, top_z], "text": "S"},
            "East":  {"pos": [upper_x1 + label_margin, uy_center, top_z], "text": "E"},
            "West":  {"pos": [upper_x0 - label_margin, uy_center, top_z], "text": "W"},
        }
    else:
        label_positions = {
