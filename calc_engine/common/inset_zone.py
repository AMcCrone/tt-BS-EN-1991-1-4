...
def detect_zone_E_and_visualise(session_state,
    ...
    This version preserves your original elevation labels and axis mapping,
    but ensures Zone-E rectangles are plotted on the explicit face that was
    recorded during the checks, so the figure always matches the dataframe.
    """
    # Colours
    ...
    # Results dict skeleton (keep labels as originally)
    results = {
    ...
    # Container for zone-E rectangles: store dicts with explicit face tag
    zoneE_rects = []  # each item: {"x0","x1","y0","y1","h","label","face"}

    # ---- For North/South elevations: B1 = EW dimension - (east + west offsets) ----
    ...
    if e1_NS > 0 and east_offset < 0.2 * e1_NS and north_offset > 0:
        ...
        if clamped:
            ...
            zoneE_rects.append({"x0": clamped[0], "x1": clamped[1],
                                "y0": clamped[2], "y1": clamped[3],
                                "h": rect_h, "label": "North-east", "face": "North"})
    ...
    if e1_NS > 0 and west_offset < 0.2 * e1_NS and north_offset > 0:
        ...
        if clamped:
            ...
            zoneE_rects.append({"x0": clamped[0], "x1": clamped[1],
                                "y0": clamped[2], "y1": clamped[3],
                                "h": rect_h, "label": "North-west", "face": "North"})
    ...
    if e1_NS > 0 and east_offset < 0.2 * e1_NS and south_offset > 0:
        ...
        if clamped:
            ...
            zoneE_rects.append({"x0": clamped[0], "x1": clamped[1],
                                "y0": clamped[2], "y1": clamped[3],
                                "h": rect_h, "label": "South-east", "face": "South"})
    ...
    if e1_NS > 0 and west_offset < 0.2 * e1_NS and south_offset > 0:
        ...
        if clamped:
            ...
            zoneE_rects.append({"x0": clamped[0], "x1": clamped[1],
                                "y0": clamped[2], "y1": clamped[3],
                                "h": rect_h, "label": "South-west", "face": "South"})
    ...
    if e1_EW > 0 and north_offset < 0.2 * e1_EW and east_offset > 0:
        ...
        if clamped:
            ...
            zoneE_rects.append({"x0": clamped[0], "x1": clamped[1],
                                "y0": clamped[2], "y1": clamped[3],
                                "h": rect_h, "label": "East-north", "face": "East"})
    ...
    if e1_EW > 0 and south_offset < 0.2 * e1_EW and east_offset > 0:
        ...
        if clamped:
            ...
            zoneE_rects.append({"x0": clamped[0], "x1": clamped[1],
                                "y0": clamped[2], "y1": clamped[3],
                                "h": rect_h, "label": "East-south", "face": "East"})
    ...
    if e1_EW > 0 and north_offset < 0.2 * e1_EW and west_offset > 0:
        ...
        if clamped:
            ...
            zoneE_rects.append({"x0": clamped[0], "x1": clamped[1],
                                "y0": clamped[2], "y1": clamped[3],
                                "h": rect_h, "label": "West-north", "face": "West"})
    ...
    if e1_EW > 0 and south_offset < 0.2 * e1_EW and west_offset > 0:
        ...
        if clamped:
            ...
            zoneE_rects.append({"x0": clamped[0], "x1": clamped[1],
                                "y0": clamped[2], "y1": clamped[3],
                                "h": rect_h, "label": "West-south", "face": "West"})
    ...
    # Draw each Zone E using the explicit face recorded earlier
